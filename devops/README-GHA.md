...
2. Go to your repository at [ascomufac/portal-ufac-volto](https://github.com/ascomufac/portal-ufac-volto).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `ufac.br` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | ufac.br | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/ascomufac/portal-ufac-volto/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/ascomufac/portal-ufac-volto/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/ascomufac/portal-ufac-volto-backend`) and Frontend (`ghcr.io/ascomufac/portal-ufac-volto-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of ufac.br](https://github.com/ascomufac/portal-ufac-volto/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/ufac.br.yml` stack.
