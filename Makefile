...
PROJECT_NAME=portal-ufac-volto
STACK_NAME=ufac-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://portal-ufac-volto.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t ascomufac/portal-ufac-volto-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t ascomufac/portal-ufac-volto-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name portal-ufac-volto-frontend-acceptance --link portal-ufac-volto-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ascomufac/portal-ufac-volto-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name portal-ufac-volto-backend-acceptance -d ascomufac/portal-ufac-volto-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop portal-ufac-volto-frontend-acceptance
	@docker stop portal-ufac-volto-backend-acceptance
